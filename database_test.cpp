...
TEST(UserConnector_create, Assert_True) {
  UserConnector testcon("localhost", "vladimir", "12345678", "Kinopoiskdb",
                        3306);
  EXPECT_TRUE(true);
}
...
TEST(UserConnector_EditUser, Assert_True) {
  mapstr data = {
      {"email", "mail@mail.ru"}, {"username", "Neymar"}, {"password", "1899"}};
  UserConnector testcon("localhost", "vladimir", "12345678", "Kinopoiskdb",
                        3306);
  string email = "mail@mail.ru";
  string username = "Lionel Messi";
...
TEST(UserConnector_EstimateFilm_GetEstimation, Assert_True) {
  ...
  vector<mapstr> exp = {{{"estimate", "5"}, {"film_idfilm", "1"}},
                        {{"estimate", "8"}, {"film_idfilm", "2"}}};
  // mapstr result = testcon.GetUserInfo(stoi(id), columns);
  EXPECT_EQ(exp, est);
...
TEST(UserConnector_AddUserPreference, Assert_True) {
  vector<mapstr> exp{{{"genre", "action"}},
                     {{"genre", "comedy"}},
                     {{"format", "full"}},
                     {{"director", "guy richie"}}};
  UserConnector testcon("localhost", "vladimir", "12345678", "Kinopoiskdb",
